...
プログラムを修正した後、その変更を公開ページに反映させる手順です。
...
# 2. 変更内容を保存 (メッセージは変更点に合わせて変えてOK)
git commit -m "ランキング機能と新コマンドblockを追加"
...
これだけでOKです！
数分後に公開URLにアクセスしなおせば、最新版に更新されます。
（更新されない場合は、ブラウザのキャッシュ削除や `Ctrl+F5` スーパーリロードを試してください）
